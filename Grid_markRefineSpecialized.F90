...
!! NOTES
!! 
!!  This collection of routines has not yet been tested and should be
!! used only as a guideline for a user's implementation.
!!
!!***
...
! Arguments
  ...
  select case (criterion)
     case(THRESHOLD)
        var = int(specs(1))
        icmp = int(specs(3))
        call gr_markVarThreshold(var,specs(2),icmp,lref,.false.)
     case(ELLIPSOID)
