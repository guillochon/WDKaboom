...
CONTAINS
    ...
    FUNCTION newt_wd_mass_1d(x)
        ...
        write (int_to_str, '(e15.9)') m(core_pos)
        call Logfile_stampMessage(myPE,'WD core mass: ' // trim(adjustl(int_to_str)))
        write (int_to_str, '(e15.9)') radius(core_pos)
        call Logfile_stampMessage(myPE,'Core radius: ' // trim(adjustl(int_to_str)))
